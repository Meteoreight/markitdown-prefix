...
It exposes one tool: `convert_to_markdown(uri: str, prefix: str | None = None)`.
*   `uri`: The URI of the resource to convert (e.g., `http:`, `https:`, `file:`, or `data:`).
*   `prefix` (optional): A string to prepend to the `uri`. If `prefix` is `null` or an empty string, it is ignored. This is useful for constructing full URIs from relative paths or partial identifiers.

For example, to process `https://example.com/docs/mypage.html`, you could send:
```json
{
